...
const find_user_resolver = async (obj, args, context, info) => {
  ...
  console.log(
    `authentication_token - ${authentication_token} - args.token = ${args.token}`
  );

  const is_authenticated_user =
    args.is_authenticate ||
  ...
  let user = {};
  if (user_id) {
    user = await context.read_db.query(
      `SELECT * FROM user WHERE user_id = ${user_id};`,
      { type: context.read_db.QueryTypes.SELECT }
    );
  ...
  if (user.length) {
    const is_authorized_user =
      args.is_authorize || (await authorize_user(null, user[0].roles));
